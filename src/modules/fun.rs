...
impl<'a> module::Module for Module<'a> {
    ...
    fn command_description(&self, id: u32) -> &'static str {
        match id {
            x if x == Commands::Fraktur as u32 => "Prints the given text in 𝔣𝔯𝔞𝔨𝔱𝔲𝔯 (gothic math symbols).",
            x if x == Commands::Temperature as u32 => "Converts the temperature between Celsius and Fahrenheit.",
            ...
            x if x == Commands::Aesthetic as u32 => "Prints the given text in ｆｕｌｌｗｉｄｔｈ characters.",
            x if x == Commands::Smallcaps as u32 => "Converts capital letters to ꜱᴍᴀʟʟ ᴄᴀᴘɪᴛᴀʟ letters.",
    ...
    fn command_help_message(&self, id: u32) -> &'static str {
        match id {
            x if x == Commands::Fraktur as u32 => "`!fraktur <text>` - Prints the given text in 𝔣𝔯𝔞𝔨𝔱𝔲𝔯 (gothic math symbols). Note that there are no regular versions of letters 'C', 'H', 'I', 'R', 'Z'; those are replaced with their bold versions.",
            x if x == Commands::Temperature as u32 => "`!temperature <number> <C or F>` - Converts the temperature into another scale. For example, `!temp 5C` outputs 41.",
            x if x == Commands::Roll as u32 => "`!roll [high]` - Prints a random number between 0 and 99, or between 0 and high - 1, inclusive.",
            x if x == Commands::Pick as u32 => "`!pick something;something else[;third option[;...]]` - Randomly picks one of the given options.",
            x if x == Commands::Info as u32 => "`!information` - Prints out some information about the server.",
            x if x == Commands::Room as u32 => "`!room <user or role mention(-s)>` - Makes a private voice room for you and mentioned users. The room is __NOT YET__ automatically deleted after a certain amount of time when everyone leaves it.",
            x if x == Commands::Aesthetic as u32 => "`!aesthetic <text>` - Prints the given text in ｆｕｌｌｗｉｄｔｈ characters.",
            x if x == Commands::Smallcaps as u32 => "`!smallcaps <text>` - Converts capital letters to ꜱᴍᴀʟʟ ᴄᴀᴘɪᴛᴀʟ letters. Note that there are no small capital versions of letters 'Q' and 'X'.",
...
impl<'a> Module<'a> {
    fn remember_command_message(&self, channel_id: ChannelId, command_id: MessageId, author: UserId, output_id: MessageId) {
    ...
    fn find_command_message(&self, channel_id: ChannelId, command_id: MessageId) -> Option<CommandMessage> {
        self.command_messages
            .read()
            .unwrap()
            .get(&channel_id)
    ...
    fn handle_fraktur(&self, bot: &Bot, message: &Message, text: &str) {
        ...
        if let Some(output) = bot.send_and_get(message.channel_id, &reply) {
            self.remember_command_message(message.channel_id, message.id, message.author.id, output.id);
        }
    }
                ...
                if message.mentions.len() > 0 || message.mention_roles.len() > 0 {
                    ...
                    match bot.create_channel(server.id,
                        ...
                        Ok(Channel::Public(new_channel)) => {
                            ...
                            bot.create_permissions(new_channel.id,
                                                   PermissionOverwrite {
