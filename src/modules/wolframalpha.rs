					...
					match event {
						Ok(XmlEvent::StartElement { name, attributes, namespace: _ }) => {
							match name.local_name.as_ref() {
								...
								"pod" => {
									for attr in attributes {
										if (attr.name.local_name == "title" && (attr.value != "Input" && attr.value != "Input interpretation"))
								...
								"img" => {
									for attr in attributes {
										if attr.name.local_name == "src" {
											...
											match pod {
												&mut Some(ref mut x) => x.image_url = Some(attr.value.clone()),
												&mut None => *pod = Some(Pod { image_url: Some(attr.value.clone()), plaintext: "".to_string() })
											}
										}
									}
								},
						...
						Ok(XmlEvent::Characters(string)) => {
							if inside_plaintext {
								...
								match pod {
									...
									&mut None => *pod = Some(Pod { image_url: None, plaintext: string.clone() })
								}
